...
class SocketService: NSObject {
    ...
    func getChannel(completion: @escaping CompletionHandler){
        socket.on("channelCreated") { (daraArray, ack) in
            guard let channelName = daraArray[0] as? String else {return}
            guard let channelDescription = daraArray[1] as? String else {return}
            guard let channelId = daraArray[2] as? String else {return}
